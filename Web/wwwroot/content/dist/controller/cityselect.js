layui.define(['jquery', 'form'], function (exports) {
    ...
    var cityselect = {
        ...
        dataArr: [],//获取数据信息
        setValues: [],//高亮状态
        Tips: ["请选择数据值", "请选择数据值", "请选择数据值"],//单选款的提示
        InitSelect: false,
        ...
        unite: function () {//进行数据的渲染
            ...
            if (that.elem.length == "2") {
                if (!that.InitSelect) {
                    provhtml += "<option value>" + that.Tips[0] + "</option>";
                }
        ...
        getValues1: function () {//只传入省
            ...
            function relaypro(citydata) {//根据省，渲染市和县
                ...
                if (citydata.length > 0) {
                    ...
                    $(that.elem[1]).html('');
                    $(that.elem[2]).html('');
                    $(that.elem[1]).append('<option value="">' + that.Tips[1]+'</option>');
                    $(that.elem[1]).append(cityhtml);
                    form.render('select',that.elembox);
                    $(that.elem[2]).append('<option value="">' + that.Tips[2]+'</option>');
                    $(that.elem[2]).append(xianhtml);
                    form.render('select',that.elembox);
                } else {
                    $(that.elem[1]).html('');
            ...
            form.on('select(' + provid + ')', function (data) {//监听省份
                ...
                function cityxian(citydata) {//重新渲染城市和县区
                    ...
                    if (citydata.length > 0) {
                        ...
                        city.html('');
                        xian.html('');
                        city.append('<option value="">' + that.Tips[1]+'</option>');
                        xian.append('<option value="">' + that.Tips[2]+'</option>');
                        city.append(city2);
                        xian.append(xian2);
                        form.render('select',that.elembox);
                    } else {
        ...
        watch2: function () {
            ...
            form.on('select(' + provid + ')', function (data) {//监听省份
                ...
                function cityxian(citydata) {//重新渲染城市
                    var city2 = '';
                    var xian2 = '';
                    if (citydata.length > 0) {
                        for (var i = 0; i < citydata.length; i++) {
